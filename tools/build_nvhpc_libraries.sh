...
case "${run_tests}" in
  ON|OFF) ;;
  *)
    echo "ERROR: WW3_LIB_RUN_TESTS must be ON or OFF (found '${run_tests}')." >&2
    exit 1
    ;;
esac
...
for command_name in cmake tar nvc nvfortran; do
...
configure_build_install "NetCDF-C ${NETCDF_C_VERSION}" \
  "${source_dir}/netcdf-c-${NETCDF_C_VERSION}" \
  "${build_root}/netcdf-c-${NETCDF_C_VERSION}" \
  -DCMAKE_C_COMPILER=nvc \
  -DCMAKE_BUILD_TYPE="${build_type}" \
  -DCMAKE_INSTALL_PREFIX="${install_prefix}" \
  -DCMAKE_PREFIX_PATH="${prefix_path}" \
  -DCMAKE_INSTALL_RPATH="${install_rpath}" \
  -DBUILD_SHARED_LIBS=ON \
  -DNETCDF_ENABLE_TESTS="${run_tests}" \
...
configure_build_install "NetCDF-Fortran ${NETCDF_FORTRAN_VERSION}" \
  ...
  -DDISABLE_ZSTANDARD_PLUGIN=TRUE
...
for library_dir in "${install_prefix}/lib" "${install_prefix}/lib64"; do
  if [[ -d "${library_dir}" ]]; then
    netcdf_link_dirs+=("-L${library_dir}")
  fi
done
nvfortran -I"${install_prefix}/include" "${script_dir}/check_netcdf.f90" \
  "${netcdf_link_dirs[@]}" \
  -lnetcdff -lnetcdf -o "${build_root}/check_netcdf"
"${build_root}/check_netcdf"

echo "Build WW3 with:"
echo "  WW3_NETCDF_ROOT=${install_prefix} ./tools/build_nvhpc.sh"
