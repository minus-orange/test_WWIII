...
case "${jobs}" in
  ""|*[!0-9]*|0) echo "ERROR: WW3_JOBS must be a positive integer." >&2; exit 2 ;;
esac
...
mpi_fc="${WW3_ONEAPI_MPIFC:-mpiifx}"
for command_name in make ar icx ifx "${mpi_fc}"; do
  require_command "${command_name}"
done
for config in "${nc_config}" "${nf_config}"; do
  if [[ ! -x "${config}" ]]; then
    echo "ERROR: NetCDF configuration utility was not found: ${config}" >&2
    echo "Run tools/build_oneapi_libraries.sh first or set WW3_NETCDF_ROOT." >&2
    exit 1
  fi
done
...
mpi_fortran="$({ "${mpi_fc}" -show 2>/dev/null || "${mpi_fc}" --showme:command 2>/dev/null; } | head -n 1)"
if [[ "${mpi_fortran}" != *ifx* ]]; then
  echo "ERROR: ${mpi_fc} does not use ifx: ${mpi_fortran:-unknown}" >&2
  echo "Load Intel oneAPI compiler and Intel MPI environments first." >&2
  exit 1
fi
netcdf_fortran="$("${nf_config}" --fc)"
if [[ "${netcdf_fortran}" != *ifx* ]]; then
  echo "ERROR: NetCDF-Fortran was not built with ifx: ${netcdf_fortran}" >&2
  exit 1
fi

export WW3_ONEAPI_MPIFC="${mpi_fc}"
export WW3_LEGACY_NC_CONFIG="${nc_config}"
export WW3_LEGACY_NF_CONFIG="${nf_config}"
export NETCDF_CONFIG="${script_dir}/oneapi_netcdf_config.sh"
...
echo "Configuring WW3 Intel oneAPI legacy build (no CMake for WW3)"
